...
**Inference (deliberate Cloudflare-first choice):** ALL the bots here -- the Hollow Grid bots (`hollow-grid/`), the Packet Wastes tools (`bot.py`), and the Discord relay -- run on **Cloudflare Workers AI** via the AI Gateway. Each is an OpenAI-compatible client pointed at the gateway's compat endpoint (the `OLLAMA_BASE_URL` knob is legacy-named and its `localhost` value is only a dev fallback; `hollow-grid` also wires this as `BOT_BRAIN=gateway`). GPU/inference comes from Cloudflare (Workers AI on Unified Billing, effectively free), so there is no self-hosted GPU -- which is why the old GPU boxes were retired. This path is tested working against the gateway.

A suite of Python tools that play and probe the **Packet Wastes** MUD (a text MUD reached over WebSocket at `wss://74-208-68-248.sslip.io/ws`). The flagship is `bot.py`, an AI-driven player that explores, fights, socializes, and files bug reports; the rest create accounts, walk the tutorial, and map/diagnose the game. Inference runs on **Cloudflare Workers AI** through the AI Gateway's OpenAI-compatible endpoint (the `OLLAMA_BASE_URL` env var is legacy-named -- `bot.py` is just an OpenAI-compatible client pointed at the gateway, not a local model). GPU time comes from Cloudflare (Workers AI on Unified Billing, effectively free), so there is no self-hosted GPU; everything still connects to the **live** game server.
