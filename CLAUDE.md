...
**Inference (deliberate Cloudflare-first choice):** the Hollow Grid bots (`hollow-grid/`) run on **Cloudflare Workers AI** via the AI Gateway (`BOT_BRAIN=gateway`), so there is no self-hosted GPU to babysit -- CF GPU time on Unified Billing, effectively free. That is why the old self-hosted GPU boxes were retired. The Packet Wastes tools and the Discord relay are OpenAI-compatible clients (`OLLAMA_BASE_URL` is a legacy name) whose base URL is pointed at the same Cloudflare AI Gateway endpoint in production -- no local model, no self-hosted GPU (the localhost default is for local dev only).
